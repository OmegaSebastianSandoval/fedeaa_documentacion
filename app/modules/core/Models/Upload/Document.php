...
class Core_Model_Upload_Document
{
    ...
    public function uploadmultiple($document)
    {
        $permitidos = array("application/pdf", "application/xls", "application/doc", "application/docx", "application/vnd.ms-excel", "application/msword","application/octet-stream","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        $limite_kb = 1000000;
        $documents = array();
        foreach($_FILES[$document]["tmp_name"] as $key=>$tmp_name){
            if ((in_array($_FILES[$document]['type'][$key], $permitidos) && $_FILES[$document]['size'][$key] <= $limite_kb * 1024)|| 1==1) {
                $filename = ''.pathinfo($_FILES[$document]['name'][$key], PATHINFO_FILENAME);
                $filename = $this->clearName($filename);
                $extension = pathinfo($_FILES[$document]['name'][$key], PATHINFO_EXTENSION);
                $name = $filename.'.'.$extension;
                $ruta = FILE_PATH .$name ;
                if (file_exists($ruta)) {
                    $increment = 0;
                ...
                if (move_uploaded_file($_FILES[$document]['tmp_name'][$key], $ruta)) {
                    $documents[$key] = $name;
                }
            }
        }
