...
$info = pathinfo($path);
if (isset($info['extension']) && !(isset($_GET['action']) && $_GET['action']=='delete_page') && !in_array(strtolower($info['extension']), $ext) && $_GET['action'] != 'create_file')
{
    ...
    switch($_GET['action'])
    {
        ...
        case 'create_page':
            if ($create_pages)
            {
                ...
                $path .= $name;
                $path_thumb .= $name;
                create_page(fix_path($path,$transliteration,$convert_spaces, $replace_with),fix_path($path_thumb,$transliteration,$convert_spaces, $replace_with));
            }
            break;
        case 'rename_page':
            if ($rename_pages){
                ...
                if (!empty($name)){
                    if (!rename_page($path,$name,$transliteration,$convert_spaces))
					{
						response(trans('Rename_existing_page'), 403)->send();
						exit;
					}

                    rename_page($path_thumb,$name,$transliteration,$convert_spaces);
                    if ($fixed_image_creation){
                        foreach($fixed_path_from_filemanager as $k=>$paths){
                            if ($paths!="" && $paths[strlen($paths)-1] != "/") $paths.="/";

                            $base_dir=$paths.substr_replace($path, '', 0, strlen($current_path));
                            rename_page($base_dir,$name,$transliteration,$convert_spaces);
                        }
                    }
                }
                else {
        ...
        case 'paste_clipboard':
            ...
            // user wants to paste page to it's own sub page.. baaaah.
        ...
        case 'chmod':
            ...
            $valid_options = array('none', 'files', 'pages', 'both');
            $chmod_perm = (is_dir($path) ? $chmod_dirs : $chmod_files);

            // check perm
            if ($chmod_perm === FALSE) {
                response(sprintf(trans('File_Permission_Not_Allowed'), (is_dir($path) ? lcfirst(trans('pages')) : lcfirst(trans('Files')) )), 403)->send();
				exit;
            }
