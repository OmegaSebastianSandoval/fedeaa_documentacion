...
if(isset($_GET['action']))
{
    switch($_GET['action'])
    {
		...
		case 'extract':
			...
			$info = pathinfo($path);
			$base_page = $current_path . fix_dirname($_POST['path']) . "/";
				...
				case "zip":
					...
					if ($zip->open($path) === true)
					{
						...
						for ($i = 0; $i < $zip->numFiles; $i++)
						{
							...
							{
								create_page($base_page . $FullFileName['name']);
							}
						}
						//unzip into the pages
						for ($i = 0; $i < $zip->numFiles; $i++)
						{
							$OnlyFileName = $zip->getNameIndex($i);
							$FullFileName = $zip->statIndex($i);
							...
							if ( ! (substr($FullFileName['name'], -1, 1) == "/"))
							{
								...
								{
									copy('zip://' . $path . '#' . $OnlyFileName, $base_page . $FullFileName['name']);
								}
							}
						}
						$zip->close();
		...
		case 'copy_cut':
			...
			if (is_dir($path))
			{
				...
				{
					response(sprintf(trans('Copy_Cut_Not_Allowed'), ($_POST['sub_action'] == 'copy' ? lcfirst(trans('Copy')) : lcfirst(trans('Cut'))), trans('pages')), 403)->send();
					exit;
				}
				
				// size over limit 
				if ($copy_cut_max_size !== false && is_int($copy_cut_max_size)) {
					list($sizepageToCopy,$fileNum,$pagesCount) = page_info($path);
					if (($copy_cut_max_size * 1024 * 1024) < $sizepageToCopy) {
		...
		case 'chmod':
			...
			{
				response(sprintf(trans('File_Permission_Not_Allowed'), (is_dir($path) ? lcfirst(trans('pages')) : lcfirst(trans('Files'))), 403), 400)->send();
				exit;
			}
			else
			{
				...
				if (is_dir($path))
				{
					$ret .= '<div class="hero-unit" style="padding:10px;">'.trans('File_Permission_Recursive').'<br/><br/>
							<ul class="unstyled">
								...
								<li><label class="radio"><input value="pages" name="apply_recursive" type="radio"> '.trans('pages').'</label></li>
								<li><label class="radio"><input value="both" name="apply_recursive" type="radio"> '.trans('Files').' & '.trans('pages').'</label></li>
							</ul>
							</div>';
				}
