...
// If hidden pages are specified
if(count($hidden_pages)){
	// If hidden page appears in the path specified in URL parameter "fldr"
	$dirs = explode('/', $subdir);
	foreach($dirs as $dir){
		if($dir !== '' && in_array($dir, $hidden_pages)){
			// Ignore the path
			$subdir = "";
			break;
		}
...
if (!isset($_SESSION['RF']["subpage"]))
{
	$_SESSION['RF']["subpage"] = '';
}
$rfm_subpage = '';

if (!empty($_SESSION['RF']["subpage"]) && strpos($_SESSION['RF']["subpage"],'../') === FALSE
   && strpos($_SESSION['RF']["subpage"],'./') === FALSE && strpos($_SESSION['RF']["subpage"],"/") !== 0
   && strpos($_SESSION['RF']["subpage"],'.') === FALSE)
{
	$rfm_subpage = $_SESSION['RF']['subpage'];
}

if ($rfm_subpage != "" && $rfm_subpage[strlen($rfm_subpage)-1] != "/") { $rfm_subpage .= "/"; }

if (!file_exists($current_path.$rfm_subpage.$subdir))
{
    $subdir = '';
    if (!file_exists($current_path.$rfm_subpage.$subdir))
    {
...
{
    $cur_dir 	 = $upload_dir . $rfm_subpage.$subdir;
    $cur_path 	 = $current_path . $rfm_subpage.$subdir;
    $thumbs_path = $thumbs_base_path. $rfm_subpage;
    $parent 	 = $rfm_subpage.$subdir;
}
...
{
    create_page(FALSE, $thumbs_path.$subdir);
}
			...
			onReady: function() {
	      	...
	      	onSave: function(imageID, newURL) {
				...
				$.ajax({
					...
					data: { url: newURL, path:$('#sub_page').val()+$('#fldr_value').val(), name:$('#aviary_img').attr('data-name') }
				}).done(function( msg ) {
					featherEditor.close();
					d = new Date();
...
<body>
	...
	<input type="hidden" id="insert_page_name" value="<?php echo trans('Insert_page_Name'); ?>" />
	<input type="hidden" id="new_page" value="<?php echo trans('New_page'); ?>" />
	...
	<input type="hidden" id="sub_page" value="<?php echo $rfm_subpage; ?>"/>
	...
	<input type="hidden" id="lang_pages" value="<?php echo trans('pages'); ?>" />
...
$files = scandir($current_path.$rfm_subpage.$subdir);
$n_files=count($files);
...
$current_pages_number = 0;
foreach($files as $k=>$file){
    if($file==".") $current_page=array('file'=>$file);
    elseif($file=="..") $prev_page=array('file'=>$file);
    elseif(is_dir($current_path.$rfm_subpage.$subdir.$file)){
		$date=filemtime($current_path.$rfm_subpage.$subdir. $file);
		if($show_page_size){
			list($size,$nfiles,$npages) = page_info($current_path.$rfm_subpage.$subdir. $file);
		...
		$file_path=$current_path.$rfm_subpage.$subdir.$file;
...
$files=array_merge(array($prev_page),array($current_page),$sorted);
?>
<!-- header div start -->
    ...
    <div class="navbar-inner">
		...
		<div class="filters">
			...
			<div class="span4 half">
			    ...
			    <?php } ?>
			    <?php if($create_pages){ ?>
						    <button class="tip btn new-page" title="<?php echo  trans('New_page')?>"><i class="icon-plus"></i><i class="icon-page-open"></i></button>
			    <?php } ?>
...
<!-- header div end -->
	...
	<li><small class="hidden-phone">(<span id="files_number"><?php echo $current_files_number."</span> ".trans('Files')." - <span id='pages_number'>".$current_pages_number."</span> ".trans('pages'); ?>)</small></li>
	</ul>
    </div>
    <!-- breadcrumb div end -->
    <div class="row-fluid ff-container">
	<div class="span12">
	    <?php if(@opendir($current_path.$rfm_subpage.$subdir)===FALSE){ ?>
	    <br/>
	    <div class="alert alert-error">There is an error! The upload page there isn't. Check your config.php file. </div>
	    <?php }else{ ?>
	    <h4 id="help"><?php echo trans('Swipe_help'); ?></h4>
	    <?php if(isset($page_message)){ ?>
		<div class="alert alert-block"><?php echo $page_message; ?></div>
	    <?php } ?>
		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
			if($file == '.' || (isset($file_array['extension']) && $file_array['extension']!=trans('Type_dir')) || ($file == '..' && $subdir == '') || in_array($file, $hidden_pages) || ($filter!='' && $n_files>$file_number_limit_js && $file!=".." && stripos($file,$filter)===false))
			  continue;
			$new_name=fix_filename($file,$transliteration);
			if($file!='..' && $file!=$new_name){
			    //rename
			    rename_page($current_path.$subdir.$file,$new_name,$transliteration);
			    $file=$new_name;
			}
			//add in thumbs page if not exist
			if (!file_exists($thumbs_path.$subdir.$file)) create_page(false,$thumbs_path.$subdir.$file);
			$class_ext = 3;
			if($file=='..' && trim($subdir) != '' ){
		    $src = explode("/",$subdir);
			...
			?>
			    ...
			    <?php if($file==".."){ ?>
			    	<input type="hidden" class="path" value="<?php echo str_replace('.','',dirname($rfm_subpage.$subdir)); ?>"/>
			    	<input type="hidden" class="path_thumb" value="<?php echo dirname($thumbs_path.$subdir)."/"; ?>"/>
			    <?php } ?>
				  <a class="page-link" href="dialog.php?<?php echo $get_params.rawurlencode($src)."&".uniqid() ?>">
					  <div class="img-precontainer">
							<div class="img-container directory"><span></span>
							<img class="directory-img"  src="img/<?php echo $icon_theme; ?>/page<?php if($file==".."){ echo "_back"; }?>.png" />
							</div>
					  </div>
			...
			<?php }else{ ?>
				    ...
				    <div class="box">
					<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a class="page-link" data-file="<?php echo $file ?>" href="dialog.php?<?php echo $get_params.rawurlencode($src)."&".uniqid() ?>"><?php echo $file; ?></a></h4>
				    </div>
				    ...
				    <?php if($show_page_size){ ?>
					    ...
					    <input type="hidden" class="npages" value="<?php echo $file_array['npages'];  ?>"/>
				    <?php } ?>
				    <div class='file-extension'><?php echo trans('Type_dir'); ?></div>
				    <figcaption>
					    <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if($rename_pages && !$file_prevent_rename) echo "rename-page"; ?>" title="<?php echo trans('Rename')?>" data-path="<?php echo $rfm_subpage.$subdir.$file; ?>"">
					    <i class="icon-pencil <?php if(!$rename_pages || $file_prevent_rename) echo 'icon-white'; ?>"></i></a>
					    <a href="javascript:void('')" class="tip-left erase-button <?php if($delete_pages && !$file_prevent_delete) echo "delete-page"; ?>" title="<?php echo trans('Erase')?>" data-confirm="<?php echo trans('Confirm_page_del'); ?>" data-path="<?php echo $rfm_subpage.$subdir.$file; ?>" >
					    <i class="icon-trash <?php if(!$delete_pages || $file_prevent_delete) echo 'icon-white'; ?>"></i>
					    </a>
				    </figcaption>
			<?php } ?>
			    </figure>
			...
			$file=$file_array['file'];
			    ...
			    if($file == '.' || $file == '..' || is_dir($current_path.$rfm_subpage.$subdir.$file) || in_array($file, $hidden_files) || !in_array(fix_strtolower($file_array['extension']), $ext) || ($filter!='' && $n_files>$file_number_limit_js && stripos($file,$filter)===false))
				    continue;
				...
				$file_path1=($current_path.$rfm_subpage.$subdir.$file1);
				if(file_exists($file_path1)){
				    $i = 1;
				    $info=pathinfo($file1);
				    while(file_exists($current_path.$rfm_subpage.$subdir.$info['filename'].".[".$i."].".$info['extension'])) {
					    $i++;
				    }
			    ...
			    try {
			    	if(!create_img($file_path, $src_thumb, 122, 91)){
							...
							new_thumbnails_creation($current_path.$rfm_subpage.$subdir,$file_path,$file,$current_path,'','','','','','','',$fixed_image_creation,$fixed_path_from_filemanager,$fixed_image_creation_name_to_prepend,$fixed_image_creation_to_append,$fixed_image_creation_width,$fixed_image_creation_height,$fixed_image_creation_option);
						}
			    } catch (Exception $e) {
						$src_thumb=$mini_src="";
			    }
				...
				if($img_width<122 && $img_height<91){
					$src_thumb=$current_path.$rfm_subpage.$subdir.$file;
					$show_original=true;
				}

				if($img_width<45 && $img_height<38){
				    $mini_src=$current_path.$rfm_subpage.$subdir.$file;
...
?>
            ...
            ?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<figcaption>
					...
					<input type="hidden" name="path" value="<?php echo $rfm_subpage.$subdir?>"/>
					...
					title="<?php echo trans('Preview')?>" data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename; ?>&file=<?php echo $rfm_subpage.$subdir.$file; ?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					    ...
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename; ?>&file=<?php echo $rfm_subpage.$subdir.$file; ?>"
						href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
						<?php }elseif($googledoc_enabled && in_array($extension_lower,$googledoc_file_exts)){ ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename; ?>&file=<?php echo $rfm_subpage.$subdir.$file; ?>"
						href="docs.google.com;" ><i class=" icon-eye-open"></i></a>

						<?php }elseif($viewerjs_enabled && in_array($extension_lower,$viewerjs_file_exts)){ ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=viewerjs&title=<?php echo $filename; ?>&file=<?php echo $rfm_subpage.$subdir.$file; ?>"
						href="docs.google.com;" ><i class=" icon-eye-open"></i></a>

				    <?php }else{ ?>
				    <a class="preview disabled"><i class="icon-eye-open icon-white"></i></a>
				    <?php } ?>
				    <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if($rename_files && !$file_prevent_rename) echo "rename-file"; ?>" title="<?php echo trans('Rename')?>" data-path="<?php echo $rfm_subpage.$subdir .$file; ?>">
 				    <i class="icon-pencil <?php if(!$rename_files || $file_prevent_rename) echo 'icon-white'; ?>"></i></a>

				    <a href="javascript:void('')" class="tip-left erase-button <?php if($delete_files && !$file_prevent_delete) echo "delete-file"; ?>" title="<?php echo trans('Erase')?>" data-confirm="<?php echo trans('Confirm_del'); ?>" data-path="<?php echo $rfm_subpage.$subdir.$file; ?>">
